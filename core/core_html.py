...
class HTML_JS:
    ...
    """)
    # 1. Логика всплывания подсказки над математическим выражением
    MathSpan = StrFormat(f"""
    <script>
    (function (){{ 
    document.querySelectorAll(".{HTML_CLASS.MathSpan.value}").forEach((elm) => {{
     // Реализуем логику только для математических выражений у которых есть дом информация, 
